...
These notes are meant to assist you in reading the Red Book (see below), and emphasize important ideas that you should key on.
...
* PLRM - PostScript Language Reference Manual (link below). Treated as Sacred Writ.
...
* **operators** are built-in to the language, while **procedure** is written by the programmer; these two kind of objects have different types
...
* **dictionary** is used here, not *map*; both of these terms imply structures which link a *key* to a corresponding *value*  
