...
* for an executable array: if encountered *directly*, then treated as data (and execution is deferred); if *indirectly* (by look-up), then each element is executed in sequence. (There's 
